...
export const useUpdatMyRestaurant = () => {
  const { getAccessTokenSilently } = useAuth0();
...
export const useGetMyRestaurantOrders = () => {
  ...
  const { data: orders, isLoading } = useQuery("fetchMyRestaurantOrders", getMyRestaurantOdersRequest);
...
export const useUpdatMyRestaurantOrder = () => {
  const { getAccessTokenSilently } = useAuth0();

  const updateMyRestaurantOder = async (updateStatusOrderRequest: UpdateOrderStatusRequest) => {
    const accessToken = await getAccessTokenSilently();
  ...
  const { mutateAsync: updateRestaurantStatus, isLoading, isError, isSuccess, reset } = useMutation(updateMyRestaurantOder);
